...
const SimpleTable = (  {getUsers, classes= useStyles,values = { members: [{firstName:"Anas",lastName:"Ben Yaiche",birthYear:1990,birthPlace:"Tunis"},{firstName:"Zina",lastName:"Ben Yaiche",birthYear:1990,birthPlace:"Tunis"}] } }) => {
        ...
        {values.members.map((n,i) => {
          return (
            <TableRow key={`${i}-${n.firstName}-${n.lastName}`}>
              ...
              <TableCell align="center"><Button><Link to={`user/${i+1}/gallery`}><PhotoIcon/></Link></Button><button>Delete</button></TableCell>
            </TableRow>
          );
        })}
      </TableBody>
